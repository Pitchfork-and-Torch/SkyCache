...
**As of v1.35.0 (Village DTN):** store-and-forward bundles carry payload SHA-256, TTL, hop-limit, and custody. USB mule v2 rejects tamper. `skycache dtn` + `/api/dtn/*` are the operator surface. Sim hop-forward only. Not Starlink. Not RFC 9171. Advertised version, README badges, and `llms.txt` stay lockstep. RX doctor `legal_receive_only` remains fail-closed (v1.34.1). Not free commercial satellite broadband. No invented Mbps. See `docs/AEO.md`.
...
### C1b. DTN hop-forward - **shipped in 1.35.0 (software / sim)**
- Payload SHA-256, TTL, hop-limit, custody, integrity-gated USB mule, `skycache dtn`.
- Community notices import + Nexus MESSAGE bridge.
- **Remaining:** on-radio batman-adv forwarder (field). Not RFC 9171.
