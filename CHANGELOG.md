...
## 1.35.0 - Village DTN store-and-forward

Village DTN is no longer a rank-only queue plus un-checked mule JSON. Payload SHA-256, TTL, hop-limit, and custody now ride on Nexus bundles and USB mules. Advertised version, README badges, and `llms.txt` stay lockstep. **Not Starlink. Not RFC 9171. Not free commercial satellite broadband.** RX doctor `legal_receive_only` remains fail-closed. No invented Mbps. No dish Mbps.

### Visible upgrades
- Payload SHA-256, TTL, hop-limit, and custody on Nexus bundles (`skycache.nexus.dtn`)
- Integrity-gated USB mule v2 (`verify_mule_file`, reject tamper / hop-loop / expired)
- Mesh hop-forward with custody transfer (`skycache dtn forward-sim`, sim path)
- REQUEST bundles prefer a mesh peer with role `gateway`
- Community notices (`dtn_lite`) gain USB import + hash + Nexus MESSAGE bridge
...
- Honest scope: store-and-forward over village mesh or USB. Not Starlink. Not RFC 9171.
- Docs: `docs/V1350-SUCCESS-CRITERIA.md`
...
### Tests
- `tests/test_dtn_store_forward.py`
- `tests/test_v1342_honesty_aeo.py` still green (advertised-version lockstep)

### Upgrade from 1.34.2
1. pip install -e .
2. skycache aeo write
3. skycache library sync --skip-zero-network --apply-web
4. Deploy skycache-web so https://skycache.jonbailey.xyz/llms.txt matches v1.35.0
...
### Surface
- `skycache mission doctor|status|export|kit|seal` plus `GET /api/mission/status`
- BLE GATT profile + ATT sim (`skycache.capabilities.ble_profile`) - no live radio claim
- `skycache power calibrate` persists `data/ops/power-calibration.json` (lab default or site Wh)
- Soak protocols for rx / dual-radio / gateway / village-day
- `skycache disaster tabletop` six-station partner script (no RF)
- INA219 wiring notes + site Wh calibration in `deploy/solar-power-notes.md`

### Upgrade from 1.33.x
1. pip install -e .
2. skycache mission seal --out data/mission-kit
3. Deploy skycache-web (mission meter 100 software / field residuals later)
...
Receive-only satellite  |  unlicensed mesh  |  no commercial decrypt  |  MIT
