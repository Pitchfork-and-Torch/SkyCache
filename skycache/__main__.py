...
def cmd_dtn(args: argparse.Namespace) -> int:
    """DTN store-and-forward: doctor, mule verify, hop-forward sim."""
    from skycache.nexus.dtn_ops import (
        dtn_doctor,
        dtn_enqueue,
        dtn_export,
        dtn_import,
    ...
    sub = getattr(args, "dtn_cmd", None) or "status"
    data_dir = Path(args.data_dir) if getattr(args, "data_dir", None) else Path("data")

    if sub == "doctor":
        rep = dtn_doctor(data_dir=data_dir)
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("go_sim_dtn") else 2
    if sub == "status":
        print(json.dumps(dtn_status(data_dir=data_dir), indent=2))
        return 0
    if sub == "enqueue":
        rep = dtn_enqueue(
            data_dir=data_dir,
            kind=str(getattr(args, "kind", "message") or "message"),
            priority_class=str(getattr(args, "priority", "general") or "general"),
            destination=str(getattr(args, "dest", "*") or "*"),
            text=str(getattr(args, "text", "") or ""),
            package_id=str(getattr(args, "package_id", "") or ""),
            ttl_sec=int(getattr(args, "ttl_sec", 604800) or 604800),
            hop_limit=int(getattr(args, "hop_limit", 8) or 8),
        )
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    if sub == "export":
        ...
        return 0 if rep.get("ok") else 1
    if sub == "import":
        rep = dtn_import(path=Path(args.path), data_dir=data_dir)
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    if sub == "verify":
        path = Path(args.path) if getattr(args, "path", None) else None
        rep = dtn_verify(path=path, data_dir=data_dir)
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    if sub == "forward-sim":
        rep = run_forward_sim(
        ...
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    print(json.dumps({"error": f"unknown dtn cmd {sub}"}, indent=2))
    return 1
...
def cmd_power(args: argparse.Namespace) -> int:
    ...
    if sub == "calibrate":
        ...
        lab = bool(getattr(args, "lab_default", False))
        wh = float(getattr(args, "battery_wh", 0) or (100.0 if lab else 0))
        if wh <= 0:
            print(json.dumps({"error": "battery-wh must be > 0 (or pass --lab-default)"}, indent=2))
            return 1
        rep = write_power_calibration(
            data_dir,
            battery_wh=wh,
            draw_w_normal=getattr(args, "draw_w", None),
            source="lab-default" if lab else "operator",
...
def cmd_mission(args: argparse.Namespace) -> int:
    """Software Mission Seal (v1.34): doctor, status, export, kit, seal."""
    ...
    sub = getattr(args, "mission_cmd", None) or "doctor"
    data_dir = Path(args.data_dir) if getattr(args, "data_dir", None) else Path("data")
    if sub == "doctor":
        rep = mission_doctor(data_dir=data_dir, repo_root=package_root())
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("go_software_mission") else 2
    if sub == "status":
        print(json.dumps(mission_status(data_dir=data_dir, repo_root=package_root()), indent=2))
        return 0
    if sub == "export":
        out = Path(args.out) if getattr(args, "out", None) else data_dir / "ops" / "mission-board.html"
        rep = export_mission_html(out, data_dir=data_dir, repo_root=package_root())
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    if sub in {"kit", "seal"}:
        out = Path(args.out) if getattr(args, "out", None) else data_dir / "mission-kit"
        fn = seal_software_mission if sub == "seal" else write_mission_kit
        rep = fn(
            out,
            data_dir=data_dir,
            repo_root=package_root(),
        ...
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    print(json.dumps({"error": f"unknown mission cmd {sub}"}, indent=2))
    return 1
...
def cmd_disaster(args: argparse.Namespace) -> int:
    ...
    if sub == "closeout":
        ...
        return 0 if rep.get("ok") else 1
    if sub == "tabletop":
        dest = Path(args.out) if getattr(args, "out", None) else data_dir / "ops" / "disaster-tabletop-last.json"
        rep = write_disaster_tabletop(dest, data_dir=data_dir)
        print(json.dumps(rep, indent=2))
        return 0 if rep.get("ok") else 1
    if sub == "kit":
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dtn_p = sub.add_parser(
        "dtn",
        help="DTN store-and-forward: integrity, TTL, hop-limit, mule, hop-forward sim",
    )
    dtn_p.add_argument("--data-dir", default="data")
    dtn_p.set_defaults(func=cmd_dtn, dtn_cmd="status")
    dtn_sub = dtn_p.add_subparsers(dest="dtn_cmd")

    dtnd = dtn_sub.add_parser("doctor", help="Queue integrity + mule roundtrip")
    dtnd.add_argument("--data-dir", default="data")
    dtnd.set_defaults(func=cmd_dtn, dtn_cmd="doctor")

    dtns = dtn_sub.add_parser("status", help="Pending bundles + custody snapshot")
    dtns.add_argument("--data-dir", default="data")
    dtns.set_defaults(func=cmd_dtn, dtn_cmd="status")

    dtne = dtn_sub.add_parser("enqueue", help="Queue a notice or open-content request")
    dtne.add_argument("--data-dir", default="data")
    dtne.add_argument("--kind", default="message", help="message|request|control")
    dtne.add_argument("--priority", default="general")
    dtne.add_argument("--dest", default="*", help="node id or *")
    dtne.add_argument("--text", default="", help="Notice body")
    dtne.add_argument("--package-id", default="", dest="package_id")
    dtne.add_argument("--ttl-sec", type=int, default=604800)
    dtne.add_argument("--hop-limit", type=int, default=8)
    dtne.set_defaults(func=cmd_dtn, dtn_cmd="enqueue")

    dtnx = dtn_sub.add_parser("export", help="Export pending bundles to a USB mule JSON")
    dtnx.add_argument("--data-dir", default="data")
    dtnx.add_argument("--out", default="data/nexus/mule")
    dtnx.add_argument("--limit", type=int, default=50)
    dtnx.set_defaults(func=cmd_dtn, dtn_cmd="export")

    dtni = dtn_sub.add_parser("import", help="Import a mule JSON (integrity-gated)")
    dtni.add_argument("path")
    dtni.add_argument("--data-dir", default="data")
    dtni.set_defaults(func=cmd_dtn, dtn_cmd="import")

    dtnv = dtn_sub.add_parser("verify", help="Verify mule file or local queue hashes")
    dtnv.add_argument("path", nargs="?", default="")
    dtnv.add_argument("--data-dir", default="data")
    dtnv.set_defaults(func=cmd_dtn, dtn_cmd="verify")

    dtnf = dtn_sub.add_parser("forward-sim", help="3-node chain hop-forward lab (no RF)")
    dtnf.add_argument("--nodes", type=int, default=3)
    dtnf.add_argument("--rounds", type=int, default=2)
    dtnf.set_defaults(func=cmd_dtn, dtn_cmd="forward-sim")
    ...
    msn = sub.add_parser(
        "mission",
        help="Software Mission Seal: doctor, status, export, kit, seal (honest 100% software meter)",
    )
    msn_sub = msn.add_subparsers(dest="mission_cmd", required=True)
    msnd = msn_sub.add_parser("doctor", help="Software mission readiness (field residuals listed, not deducted)")
    msnd.add_argument("--data-dir", default="data")
    msnd.set_defaults(func=cmd_mission, mission_cmd="doctor")
    msns = msn_sub.add_parser("status", help="Compact software mission snapshot")
    msns.add_argument("--data-dir", default="data")
    msns.set_defaults(func=cmd_mission, mission_cmd="status")
    msne = msn_sub.add_parser("export", help="Printable mission board HTML")
    msne.add_argument("--data-dir", default="data")
    msne.add_argument("--out", default="data/ops/mission-board.html")
    msne.set_defaults(func=cmd_mission, mission_cmd="export")
    msnk = msn_sub.add_parser("kit", help="Build mission kit folder + zip")
    msnk.add_argument("--data-dir", default="data")
    msnk.add_argument("--out", default="data/mission-kit")
    msnk.add_argument("--no-zip", action="store_true")
    msnk.set_defaults(func=cmd_mission, mission_cmd="kit")
    msnl = msn_sub.add_parser("seal", help="Write calibration + BLE sim + tabletop + soak + board")
    msnl.add_argument("--data-dir", default="data")
    msnl.add_argument("--out", default="data/mission-kit")
    msnl.add_argument("--no-zip", action="store_true")
    msnl.set_defaults(func=cmd_mission, mission_cmd="seal")
    ...
    pwc = pwr_sub.add_parser("calibrate", help="Persist site or lab battery_wh calibration")
    pwc.add_argument("--data-dir", default="data")
    pwc.add_argument("--battery-wh", type=float, default=0.0)
    pwc.add_argument("--draw-w", type=float, default=None)
    pwc.add_argument("--notes", default="")
    pwc.add_argument("--lab-default", action="store_true", help="Write 100 Wh lab default")
    pwc.set_defaults(func=cmd_power, power_cmd="calibrate")
    ...
    dst = dis_sub.add_parser("tabletop", help="Partner tabletop script + receipt (no RF)")
    dst.add_argument("--data-dir", default="data")
    dst.add_argument("--out", default="data/ops/disaster-tabletop-last.json")
    dst.set_defaults(func=cmd_disaster, disaster_cmd="tabletop")
