...
TABLETOP_SCHEMA = "skycache.disaster.tabletop.v1"

TABLETOP_STATIONS = [
    "Roles: facilitator, radio/legal, mule runner, portal, logbook, closeout",
    "State the legal rails out loud: receive-only, open content, unlicensed mesh only",
    "Turn disaster mode ON in sim and flood emergency/health priority",
    "Export a mule and restore it on a second sim node",
    "Confirm license passports still travel with packs",
    "Turn disaster mode OFF and write the closeout receipt",
]


def disaster_tabletop_script() -> dict[str, Any]:
    """Partner tabletop without RF. Software-complete six-station script."""
    return {
        "schema": TABLETOP_SCHEMA,
        "station_count": len(TABLETOP_STATIONS),
        "stations": list(TABLETOP_STATIONS),
...
def write_disaster_tabletop(out_path: Path | None = None, *, data_dir: Path | None = None) -> dict[str, Any]:
    settings = _settings(data_dir)
    dest = Path(out_path) if out_path else settings.data_dir / "ops" / "disaster-tabletop-last.json"
    dest.parent.mkdir(parents=True, exist_ok=True)
    body = {
        **disaster_tabletop_script(),
        "ok": True,
        "generated_at": _iso_now(),
        "software_version": __version__,
        "path": str(dest),
        "legal": "Tabletop only. Not a substitute for a real partner exercise or emergency services.",
    }
...
def disaster_doctor(
    *,
    ...
    add(
        "tabletop_script",
        int(script.get("station_count") or 0) >= 6,
        f"{script.get('station_count')} tabletop stations (no RF)",
        8,
    )
    ...
    go_tabletop = int(script.get("station_count") or 0) >= 6
