...
CAL_SCHEMA = "skycache.power.calibration.v1"
CAL_NAME = "power-calibration.json"


def calibration_path(data_dir: Path | None) -> Path:
    root = Path(data_dir) if data_dir else Path("data")
    return root / "ops" / CAL_NAME
...
def load_power_calibration(
    *,
    ...
    """Load site calibration or the software lab default (100 Wh)."""
    del repo_root  # reserved for future site templates
    path = calibration_path(data_dir)
    if path.is_file():
        try:
            raw = json.loads(path.read_text(encoding="utf-8"))
            if float(raw.get("battery_wh") or 0) > 0:
                return raw
        except (OSError, json.JSONDecodeError, TypeError, ValueError):
            pass
    return {
        "schema": CAL_SCHEMA,
        "battery_wh": DEFAULT_BATTERY_WH,
        "draw_w_normal": 6.5,
        "source": "lab-default",
        "notes": "Software lab default. Replace with site bank Wh.",
        "site_owned": False,
    }
...
def write_power_calibration(
    ...
    *,
    battery_wh: float,
    draw_w_normal: float | None = None,
    source: str = "operator",
    notes: str = "",
) -> dict[str, Any]:
    """Persist site or lab Wh calibration under data/ops/."""
    path = calibration_path(data_dir)
    ...
    body = {
        ...
        "draw_w_normal": float(draw_w_normal if draw_w_normal is not None else 6.5),
        "source": source,
        "notes": notes or "Calibrate to the installed bank. Rough guidance only.",
        "site_owned": source == "operator",
        "banner": HONEST,
    }
    path.write_text(json.dumps(body, indent=2) + "\n", encoding="utf-8")
    return {**body, "ok": True, "path": str(path)}
...
def _battery_wh(settings) -> float:
    env = getattr(settings, "battery_wh", None)
    if env:
        try:
            val = float(env)
            if val > 0:
            ...
            pass
    cal = load_power_calibration(data_dir=getattr(settings, "data_dir", None))
    return float(cal.get("battery_wh") or DEFAULT_BATTERY_WH)
...
def power_doctor(
    *,
    ...
    cal = load_power_calibration(data_dir=settings.data_dir)
    add(
        "calibration",
        float(cal.get("battery_wh") or 0) > 0,
        f"source={cal.get('source')} battery_wh={cal.get('battery_wh')}",
        10,
    )
    ...
    return {
        ...
        "go_power_calibrated": float(cal.get("battery_wh") or 0) > 0,
        "calibration": {
            "source": cal.get("source"),
            "battery_wh": cal.get("battery_wh"),
            "site_owned": bool(cal.get("site_owned")),
        },
        "status": st,
        "checks": checks,
        "banner": HONEST,
        "next_steps": [
            ...
            "skycache power calibrate --battery-wh NNN --notes 'site bank'",
