...
def handoff_doctor(*, data_dir: Path | None = None) -> dict[str, Any]:
    ...
    ble = simulate_ble_exchange({"packages": pkgs[:3], "node_id": "handoff-doctor"})
    add("ble_sim", bool(ble.get("ok")), f"GATT ATT sim frames={ble.get('frame_count')} (no live radio)", 8)
