...
class NexusSimulator:
            ...
            for b in self.nodes:
                ...
                a.mesh.upsert_peer(
                    MeshPeer(
                        ...
                        role="gateway" if b.gateway.sim_uplink else "node",
                    )
                )
    ...
    def run_round(self) -> dict[str, Any]:
        ...
        if len(self.nodes) >= 2:
            ...
            mule_path = str(self.nodes[0].dtn.export_mule(mule_dir, exporter_node=self.nodes[0].name))
            self.nodes[-1].dtn.import_mule(Path(mule_path), self.nodes[-1].name)

        peer_queues = {n.name: n.dtn for n in self.nodes}
        dtn_fwd = []
        for n in self.nodes:
            others = {k: v for k, v in peer_queues.items() if k != n.name}
            dtn_fwd.append(
                forward_round(
                    local_node=n.name,
                    local_queue=n.dtn,
