...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def nexus_status() -> dict[str, Any]:
        ...
        return {
            ...
            "phase": 4,
            "edition": f"{__version__} Village DTN",
    ...
    @app.get("/api/dtn/status")
    async def dtn_api_status() -> dict[str, Any]:
        from skycache.nexus.dtn_ops import dtn_doctor, dtn_status

        doc = dtn_doctor(data_dir=settings.data_dir)
        st = dtn_status(data_dir=settings.data_dir)
        return {**st, "doctor": doc}

    @app.post("/api/dtn/enqueue")
    async def dtn_api_enqueue(payload: dict[str, Any]) -> dict[str, Any]:
        from skycache.nexus.dtn_ops import dtn_enqueue

        kind = str(payload.get("kind") or "message")
        if kind in ("request", "control"):
            pin = str(payload.get("admin_pin") or "")
            require_admin_pin(settings.admin_pin, pin)
        return dtn_enqueue(
            data_dir=settings.data_dir,
            kind=kind,
            priority_class=str(payload.get("priority_class") or payload.get("priority") or "general"),
            destination=str(payload.get("destination") or payload.get("dest") or "*"),
            text=str(payload.get("text") or payload.get("body") or ""),
            package_id=str(payload.get("package_id") or ""),
            ttl_sec=int(payload.get("ttl_sec") or 604800),
            hop_limit=int(payload.get("hop_limit") or 8),
        )

    @app.post("/api/dtn/export-mule")
    async def dtn_api_export_mule(payload: dict[str, Any] | None = None) -> dict[str, Any]:
        from skycache.nexus.dtn_ops import dtn_export
    ...
    async def dtn_api_import_mule(payload: dict[str, Any]) -> dict[str, Any]:
        from skycache.nexus.dtn_ops import dtn_import

        path = str(payload.get("path") or "").strip()
        if not path:
            return {"ok": False, "error": "path required"}
        return dtn_import(path=Path(path), data_dir=settings.data_dir)

    @app.post("/api/dtn/verify")
    async def dtn_api_verify(payload: dict[str, Any] | None = None) -> dict[str, Any]:
        from skycache.nexus.dtn_ops import dtn_verify

        body = payload or {}
        path = str(body.get("path") or "").strip()
        return dtn_verify(path=Path(path) if path else None, data_dir=settings.data_dir)
    ...
    async def mission_ops_api_status() -> dict[str, Any]:
        """Software mission seal: 100% software meter + listed field residuals."""
        from skycache.ops.mission_ops import mission_doctor, mission_status
    ...
    async def mission_ops_api_seal(payload: dict[str, Any] | None = None) -> dict[str, Any]:
        from skycache.ops.mission_ops import seal_software_mission

        payload = payload or {}
        if payload.get("admin_pin"):
            require_admin_pin(settings.admin_pin, str(payload.get("admin_pin")))
        out = Path(payload.get("out") or (settings.data_dir / "mission-kit"))
        return seal_software_mission(out, data_dir=settings.data_dir)
    ...
    async def post_message(payload: dict[str, str]) -> dict[str, Any]:
        """Community notice queue (local store-and-forward + Nexus DTN bridge)."""
        author = (payload.get("author") or "anonymous").strip()
        subject = (payload.get("subject") or "notice").strip()
        body = (payload.get("body") or "").strip()
        dest = (payload.get("destination") or "local-community").strip()
        if not body:
            raise HTTPException(400, "body required")
        msg = dtn.enqueue(author, subject, body, destination=dest, origin_node=node_id)
        bridged = dtn.bridge_to_nexus(nexus_dtn, origin_node=node_id, message=msg)
        return {
            "id": msg.id,
            "status": "queued",
            "payload_sha256": msg.payload_sha256,
            "nexus_bundle_id": bridged[0] if bridged else None,
        }
    ...
    async def import_messages(payload: dict[str, str]) -> dict[str, Any]:
        """Import a DTN-lite USB outbox (SHA-256 gated)."""
        path = (payload.get("path") or "").strip()
        if not path:
            raise HTTPException(400, "path required")
        return dtn.import_usb(Path(path), local_node=node_id)
